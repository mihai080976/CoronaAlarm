...
{
   printf("Date of registration %d.%d.%d", a_date.day, a_date.month, a_date.year);
}
...
date_t set_to_today(date_t a_date)
{
   time_t today_in_seconds;
   struct tm * timeinfo;

   date_t a_date1 = a_date;

   today_in_seconds = time(NULL);
   timeinfo = localtime(&today_in_seconds);

   a_date1.day = timeinfo->tm_mday;
   a_date1.month = timeinfo->tm_mon +1; //tm_mon is months since January
   a_date1.year = timeinfo->tm_year + 1900; //tm_year years sinece 1900
   return a_date1;
}
